...
namespace PEScan {
    ...
    BOOL PEParser::parseDosHeader() {
        if (m_peReader->readData(0x0, &m_peStruct->dosHeader, sizeof(IMAGE_DOS_HEADER)) < 0) {
            ...
            m_logger << CodeLogInfo(_T("Error: Fail to read DOS header")) << NL;

            return FALSE;
        }
        else if (m_peStruct->dosHeader.e_magic != IMAGE_DOS_SIGNATURE) {
            m_logger << LogLevel::ERR;
            m_logger << CodeLogInfo(_T("Error: Invalid DOS header signature")) << NL;
    ...
    BOOL PEParser::parseNtHeader() {
        ...
        if (m_peReader->readData(pNtHeader, &ntHeader, sizeof(IMAGE_NT_HEADERS32)) < 0) {
            m_logger << LogLevel::ERR;
            m_logger << CodeLogInfo(_T("Error: Fail to read NT header")) << NL;

            return FALSE;
        }
        else if (ntHeader.Signature != IMAGE_NT_SIGNATURE) {
            m_logger << LogLevel::ERR;
            m_logger << CodeLogInfo(_T("Error: Invalid NT header signature")) << NL;
    ...
    BOOL PEParser::parseEAT() {
        ...
        // 등록한 동적 할당된 주소를 함수 종료시 할당해제
        reserveCall.reset([&]() {
            if (nameOrdinals != NULL) delete[] nameOrdinals;
            if (nameAddresses != NULL) delete[] nameAddresses;
            if (functionAddresses != NULL) delete[] functionAddresses;
            if (functionNamesRVA != NULL) delete[] functionNamesRVA;
        });
    ...
    BOOL PEParser::parseTLS32() {
        ...
        if (m_peReader->readData((DWORD)va, &tlsDirectory, sizeof(IMAGE_TLS_DIRECTORY32)) >= 0) {
            ...
            while ((m_peReader->readData((DWORD)curCallbackArrayAddress, &callbackAddress, sizeof(DWORD)) >= 0, TRUE) && (callbackAddress != 0x0)) {
    ...
    BOOL PEParser::parseTLS64() {
        ...
        if (m_peReader->readData(va, &tlsDirectory, sizeof(IMAGE_TLS_DIRECTORY64)) >= 0) {
            ...
            while ((m_peReader->readData((QWORD)curCallbackArrayAddress, &callbackAddress, sizeof(QWORD)) >= 0, TRUE) && (callbackAddress != 0x0)) {
